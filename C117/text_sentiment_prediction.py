...
# train_data = pd.read_csv("./static/data_files")
training_sentences = []
...
def predict(input):
    ...
    if input != "":
        ...
        text_padded = np.array(padded)
        predicted_class_label = np.argmax(model.predict(text_padded),axis=1)
    ...
    return predicted_emotion, emoji_url
